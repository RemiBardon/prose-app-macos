...
framework:
	...
	(cd "${BUILD_FOLDER}/x86_64-apple-darwin"; swiftc \
		-module-name "${SWIFT_LIB_NAME}" \
		-emit-library -o "./lib${SWIFT_LIB_NAME}.a" \
		-emit-module -emit-module-path "./" \
		-parse-as-library \
	...
	(cd "${BUILD_FOLDER}/aarch64-apple-darwin"; swiftc \
		-module-name "${SWIFT_LIB_NAME}" \
		-emit-library -o "./lib${SWIFT_LIB_NAME}.a" \
		-emit-module -emit-module-path "./" \
		-parse-as-library \
		...
		-Xcc -fmodule-map-file="../interface/${SWIFT_LIB_NAME}FFI.modulemap" \
		-static \
		-target aarch64-apple-macos12.0 \
		"../interface/${SWIFT_LIB_NAME}.swift")
	# -enable-library-evolution
	...
	# (cd "${BUILD_FOLDER}"; swiftc \
	# 	-module-name "${SWIFT_LIB_NAME}" \
	# 	-emit-library -o "./lib${SWIFT_LIB_NAME}_macos.dylib" \
	# 	-emit-module -emit-module-path "./" \
	# 	-parse-as-library \
	# 	-L "../${RUST_LIB_PATH}/target/x86_64-apple-darwin/release" \
	# 	-l "lib${RUST_LIB_NAME}.dylib" \
	# 	-Xcc -fmodule-map-file="./interface/${SWIFT_LIB_NAME}FFI.modulemap" \
	# 	-static \
	# 	"./interface/${SWIFT_LIB_NAME}.swift")
	# 	# -enable-library-evolution \
	...
	# lipo -create -output "${BUILD_FOLDER}/lib${SWIFT_LIB_NAME}_macos.a" \
	# 	"${BUILD_FOLDER}/x86_64-apple-darwin/lib${SWIFT_LIB_NAME}.a" \
	# 	"${BUILD_FOLDER}/aarch64-apple-darwin/lib${SWIFT_LIB_NAME}.a"

	# # Group iOS Simulator libraries
	# lipo -create -output "${BUILD_FOLDER}/lib${SWIFT_LIB_NAME}_iossimulator.a" \
	...
	# lipo -create -output "${BUILD_FOLDER}/lib${SWIFT_LIB_NAME}_maccatalyst.a" \
	...
	xcodebuild -create-xcframework \
		...
		-library "${BUILD_FOLDER}/x86_64-apple-darwin/lib${SWIFT_LIB_NAME}.a" \
